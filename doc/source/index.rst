...
#. Add Stack User
   ...
   Devstack should be run as a non-root user with sudo enabled (maybe
   call out the cloud-image login users such as "ubuntu" or "cloud-user").
   If not, you can create a `stack` user to run DevStack with the script.
