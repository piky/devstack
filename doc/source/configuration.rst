...
variable can be set to ``True`` (or, touch the sentinel file
``$FILES/devstack-no-image-download``) to prevent DevStack downloading
images during test-runs (i.e. ensure they are pre-cached).
