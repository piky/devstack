...
``eventlet``: This is the default. It's also **deprecated** and will be going
away in the Newton release. Keystone runs under `keystone-all` which is an
executable that starts up the admin and public services running under an
eventlet-based server.

``mod_wsgi``: In this case Apache is configured to serve keystone using the
mod_wsgi Apache module. Apache listens on :5000 and :35357, and it also sends
requests to :80/identity and :80/identity_admin to the respective keystone
service.
