...
The new configuration file is ``local.conf`` and resides in the
``devstack/samples`` directory, copy this file into the root devstack
directory where the old ``localrc`` file resides. It is a modified INI
