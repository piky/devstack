...
To enable live migration with default live migration url shown as below,
...
set up the stack user on each compute node with an ssh key for root
of each compute node for access:

::

    cat /root/.ssh/id_rsa.pub >> ~stack/.ssh/authorized_keys

The reason is that libvirtd who starts ssh session is running as root.
