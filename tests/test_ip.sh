...
function test_cidr2netmask {
    ...
    for oct1 in $ips; do
        mask=$(( mask + 1 ))
        msg="cidr2netmask(/$mask) == $oct1.0.0.0"
        assert_equal "$oct1.0.0.0" $(cidr2netmask $mask) "$msg"
    done

    for oct2 in $ips; do
        mask=$(( mask + 1 ))
        msg="cidr2netmask(/$mask) == 255.$oct2.0.0"
        assert_equal "255.$oct2.0.0" $(cidr2netmask $mask) "$msg"
    done

    for oct3 in $ips; do
        mask=$(( mask + 1 ))
        msg="cidr2netmask(/$mask) == 255.255.$oct3.0"
        assert_equal "255.255.$oct3.0" $(cidr2netmask $mask) "$msg"
    done

    for oct4 in $ips; do
        mask=$(( mask + 1 ))
        msg="cidr2netmask(/$mask) == 255.255.255.$oct4"
        assert_equal "255.255.255.$oct4" $(cidr2netmask $mask) "$msg"
    done
}
