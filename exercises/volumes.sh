...
die_if_not_zero "Failure attaching volume $VOL_NAME to $NAME"
...
die_if_not_zero "Failure detaching volume $VOL_NAME from $NAME"
if ! timeout $ACTIVE_TIMEOUT sh -c "while ! nova volume-list | grep $VOL_NAME | grep available; do sleep 1; done"; then
...
die_if_not_zero "Failure deleting volume $VOL_NAME"
