...
function confirm_server_active {
    local VM_UUID=$1
    if ! timeout $ACTIVE_TIMEOUT sh -c "while ! nova  --no-cache show $VM_UUID | grep status | grep -q ACTIVE; do sleep 1; done"; then
...
function create_vm {
    ...
    local VM_UUID=`nova --no-cache boot --flavor $(get_flavor_id m1.tiny) \
        --image $(get_image_id) \
        $NIC \
...
function ping_ip {
   ...
   local VM_NAME=$1
   IP=`nova  --no-cache show $VM_NAME | grep 'network' | awk '{print $5}'`
...
function shutdown_vm {
    ...
    nova --no-cache delete $VM_NAME
}

function shutdown_vms {
    foreach_tenant_vm 'shutdown_vm ${%TENANT%_NAME} %NUM%'
    if ! timeout $TERMINATE_TIMEOUT sh -c "while nova --no-cache list | grep -q ACTIVE; do sleep 1; done"; then
