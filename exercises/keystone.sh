...
TENANT_1_ID=`keystone tenant-create --name $TENANT_1 --enabled true | grep -v description | grep id | cut -d "|" -f3 | tr -d ' '`
TENANT_2_ID=`keystone tenant-create --name $TENANT_2 --enabled true | grep -v description | grep id | cut -d "|" -f3 | tr -d ' '`
USER_1_ID=`keystone user-create --name $USER_1 --pass password --enabled true | grep -v email | grep -v password | grep id | cut -d '|' -f3 | tr -d ' '`
USER_2_ID=`keystone user-create --name $USER_2 --pass password --enabled true | grep -v email | grep -v password | grep id | cut -d '|' -f3 | tr -d ' '`
USER_3_ID=`keystone user-create --name $USER_3 --pass password --enabled true | grep -v email | grep -v password | grep id | cut -d '|' -f3 | tr -d ' '`
USER_4_ID=`keystone user-create --name $USER_4 --pass password --enabled true | grep -v email | grep -v password | grep id | cut -d '|' -f3 | tr -d ' '`
