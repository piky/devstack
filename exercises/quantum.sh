...
function ping_ip {
   ...
   #TODO(nati) Now namesapce supprot is merged. We can't ping to the server without test-agent.
   local VM_NAME=$1
   nova  --no_cache show $VM_NAME | grep 'network' | awk '{print $5}' | xargs -I% ping -c 1 -w 1  %
}
