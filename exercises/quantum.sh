...
function add_tenant {
    ...
    $KEYSTONE user-role-add --user-id $USER_ID --role-id $(get_role_id admin) --tenant-id $TENANT_ID
    $KEYSTONE user-role-add --user-id $USER_ID --role-id $(get_role_id anotherrole) --tenant-id $TENANT_ID
}
