...
#DEMO2_VM1_NET='demo2-net1,demo2-net2'
...
function create_network {
    ...
    local NET_ID=$(quantum net-create --tenant_id $TENANT_ID $NET_NAME| grep ' id ' | awk '{print $4}' )
    quantum subnet-create --ip_version 4 --tenant_id $TENANT_ID --gateway $GATEWAY $NET_ID $CIDR
    #TDO(nati) comment out until l3-fw is merged
...
function create_networks {
    foreach_tenant_net 'create_network ${%TENANT%_NAME} ${%TENANT%_NET%NUM%_GATEWAY} ${%TENANT%_NET%NUM%_CIDR} %NUM%'
...
function ping_ip {
   ...
   #TODO (nati) generates original config file to test-agent
   python $TOP_DIR/exercises/quantum/quantumtester/test-agent.py --config-file=/etc/quantum/dhcp_agent.ini
}
...
function shutdown_vm {
    ...
    local NUM=$2
    source $TOP_DIR/openrc $TENANT $TENANT
    nova --no_cache delete ${TENANT}-server$NUM
}
