...
IMAGE=`glance -S keystone -I $OS_USERNAME -K $OS_PASSWORD -T $OS_TENANT_NAME -H $GLANCE_HOST index | egrep $DEFAULT_IMAGE_NAME | head -1 | cut -d" " -f1`
...
# FIXME (anthony) - python-novaclient should accept a volume_id for the attachment param
DEVICE=/dev/vdb
...
cd $STAGING_DIR
# Grab the whole-disk version of cirros
sudo wget http://launchpad.net/cirros/trunk/0.3.0/+download/cirros-0.3.0-x86_64-rootfs.img.gz
