...
IP=`nova show $VM_UUID | grep "$PRIVATE_NETWORK" | get_field 2`
die_if_not_set IP "Failure retrieving IP address"

ping_check "$PRIVATE_NETWORK" $IP $BOOT_TIMEOUT
...
ping_check "$PUBLIC_NETWORK" $FLOATING_IP $ASSOCIATE_TIMEOUT

#TODO(nati) : Multiple l3-agent needed to test this with quantum. So skip it for now.
