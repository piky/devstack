...
ACTIVE_TIMEOUT=120
if (( "$ACTIVE_TIMEOUT" < 120 )); then ACTIVE_TIMEOUT=120; fi
...
function await_termination {
    ...
    if ! timeout $ACTIVE_TIMEOUT sh -c "while eval nova $CMD $1 | grep status; do sleep 1; done"; then
        [ $2 ] && TYPE="${2}" || TYPE="server"
        echo "$TYPE $1 didn't go away"
        exit 1
    fi
}
