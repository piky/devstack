...
if is_service_enabled g-reg; then
    ...
    for image_url in ${IMAGE_URLS//,/ }; do
        ...
        if [ "$CONTAINER_FORMAT" = "bare" ]; then
            ...
            if [ -n "$KERNEL" ]; then
                RVAL=`glance --os-auth-token $TOKEN image-create --name "$IMAGE_NAME-kernel" --public --container-format aki --disk-format aki < "$KERNEL"`
                KERNEL_ID=`echo $RVAL | cut -d":" -f2 | tr -d " "`
            fi
            if [ -n "$RAMDISK" ]; then
                RVAL=`glance --os-auth-token $TOKEN image-create --name "$IMAGE_NAME-ramdisk" --public --container-format ari --disk-format ari < "$RAMDISK"`
