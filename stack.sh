...
Q_ADMIN_USERNAME=${Q_ADMIN_USERNAME:-admin}
...
if is_service_enabled quantum; then
    ...
    if [[ "$Q_PLUGIN" = "openvswitch" ]]; then
        ...
        if [[ "$NOVA_USE_QUANTUM_API" = "v1" ]]; then
            ...
            Q_PLUGIN_CLASS="quantum.plugins.linuxbridge.LinuxBridgePluginV2.LinuxBridgePluginV2"
        fi
    else
    ...
    if [[ "$Q_PLUGIN" = "openvswitch" && $OVS_ENABLE_TUNNELING = "True" ]]; then
        ...
        if [[ "$NOVA_USE_QUANTUM_API" = "v1" ]]; then
            sudo sed -i -e "s/.*target_v2_api = .*$/target_v2_api = False/g" /$Q_PLUGIN_CONF_FILE
        elif [[ "$NOVA_USE_QUANTUM_API" = "v2" ]]; then
            sudo sed -i -e "s/.*target_v2_api = .*$/target_v2_api = True/g" /$Q_PLUGIN_CONF_FILE
        fi
    fi
fi
...
if is_service_enabled q-dhcp; then
    AGENT_DHCP_BINARY="$QUANTUM_DIR/quantum/agent/dhcp_agent.py"
    ...
    iniset $Q_DHCP_CONF_FILE DEFAULT admin_user quantum
...
if is_service_enabled quantum; then
    if [[ "$NOVA_USE_QUANTUM_API" = "v1" ]]; then
        ...
        add_nova_opt "quantum_admin_auth_url=$KEYSTONE_SERVICE_PROTOCOL://$KEYSTONE_SERVICE_HOST:$KEYSTONE_SERVICE_PORT/v2.0"
...
if is_service_enabled mysql && is_service_enabled nova; then
    if [[ "$NOVA_USE_QUANTUM_API" = "v1" ]]; then
        ...
        # create a small network
        NET_ID=$(python $QUANTUM_CLIENT_DIR/quantumclient/shell.py create_net --os-token $Q_ADMIN_USERNAME --os-url http://$Q_HOST:$Q_PORT --tenant-id $TENANT_ID net1 | grep ' id ' | get_field 2)

        # create a subnet
        python $QUANTUM_CLIENT_DIR/quantumclient/shell.py create_subnet --os-token $Q_ADMIN_USERNAME --os-url http://$Q_HOST:$Q_PORT --tenant-id $TENANT_ID --ip-version 4 --gateway  $NETWORK_GATEWAY $NET_ID $FIXED_RANGE
    fi
fi
