...
PIDSTAT_FILE=${PIDSTAT_FILE:-"pidstat.dat"}
...
if is_service_enabled pidstat; then
    # Per-process stats
    if [[ -n ${SCREEN_LOGDIR} ]]; then
        screen_it pidstat "cd $TOP_DIR; pidstat $PIDSTAT_INTERVAL > $SCREEN_LOGDIR/$PIDSTAT_FILE"
    else
        screen_it pidstat "pidstat $PIDSTAT_INTERVAL"
    fi
fi
