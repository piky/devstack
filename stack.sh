...
if [ "$VIRT_DRIVER" = 'xenserver' ]; then
    ...
    _FNB_FROM_KARGS=$(grep -o 'flat_network_bridge=[^.]*' /proc/cmdline | cut -d= -f 2)
    FLAT_NETWORK_BRIDGE_DEFAULT=${_FNB_FROM_KARGS:-xapi1}
...
if [ "$VIRT_DRIVER" = 'xenserver' ]; then
    ...
    FIREWALL_DRIVER=${FIREWALL_DRIVER:-"nova.virt.xenapi.firewall.Dom0IptablesFirewallDriver"}
else
    add_nova_flag "--connection_type=libvirt"
    FIREWALL_DRIVER=${FIREWALL_DRIVER:-"nova.virt.libvirt.firewall.IptablesFirewallDriver"}
fi

# see above for virt specific defaults
add_nova_flag "--firewall_driver=$FIREWALL_DRIVER"
