...
if [[ "$ENABLED_SERVICES" =~ "n-api" ]]; then
    ...
    cp $NOVA_DIR/etc/nova/api-paste.ini $NOVA_CONF/nova-api-paste.ini

    # Then we add our own service token to the configuration
    sed -e "s,%SERVICE_TOKEN%,$SERVICE_TOKEN,g" -i $NOVA_CONF/nova-api-paste.ini

    # Finally, we change the pipelines in nova to use keystone
    function replace_pipeline() {
        sed "/\[pipeline:$1\]/,/\[/s/^pipeline = .*/pipeline = $2/" -i $NOVA_CONF/nova-api-paste.ini
    }
...
add_nova_flag "--api_paste_config=$NOVA_CONF/nova-api-paste.ini"
