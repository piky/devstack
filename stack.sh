...
if is_service_enabled q-l3; then
    ...
    if [[ "$Q_PLUGIN" = "openvswitch" ]]; then
        ...
        quantum_setup_ovs_bridge $PUBLIC_BRIDGE
    elif [[ "$Q_PLUGIN" = "linuxbridge" ]]; then
...
if is_service_enabled q-svc; then
    ...
    if is_service_enabled q-l3; then
        ...
        EXT_NET_ID=$(quantum net-create ext_net | grep ' id ' | get_field 2)
        EXT_GW_IP=$(quantum subnet-create --ip_version 4 $EXT_NET_ID $FLOATING_RANGE | grep 'gateway_ip' | get_field 2)
        quantum router-gateway-set $ROUTER_ID $EXT_NET_ID
        PREFIX=${FLOATING_RANGE#*/}
        sudo ip addr add $EXT_GW_IP/$PREFIX dev $PUBLIC_BRIDGE
    fi
