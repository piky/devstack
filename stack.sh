...
if is_service_enabled quantum; then
    ...
    if [[ "$Q_PLUGIN" = "metaplugin" ]]; then
        mkdir -p /$OVS_PLUGIN_CONF_PATH
        mkdir -p /$LB_PLUGIN_CONF_PATH
        OVS_PLUGIN_CONF_FILE=$OVS_PLUGIN_CONF_PATH/$OVS_PLUGIN_CONF_FILENAME
        cp $QUANTUM_DIR/$OVS_PLUGIN_CONF_FILE /$OVS_PLUGIN_CONF_FILE
        sudo sed -i -e "s/^sql_connection =.*$//g" /$OVS_PLUGIN_CONF_FILE
        iniset /$OVS_PLUGIN_CONF_FILE DATABASE sql_connection "mysql://$MYSQL_USER:$MYSQL_PASSWORD@$MYSQL_HOST/$Q_DB_NAME?charset=utf8"
        LB_PLUGIN_CONF_FILE=$LB_PLUGIN_CONF_PATH/$LB_PLUGIN_CONF_FILENAME
        cp $QUANTUM_DIR/$LB_PLUGIN_CONF_FILE /$LB_PLUGIN_CONF_FILE
        iniset /$LB_PLUGIN_CONF_FILE DATABASE sql_connection "mysql://$MYSQL_USER:$MYSQL_PASSWORD@$MYSQL_HOST/$Q_DB_NAME?charset=utf8"
    fi
...
if [[ "$Q_PLUGIN" = "metaplugin" ]]; then
    ...
    screen_it q-agt-ovs "sudo python $OVS_AGENT_BINARY --config-file $Q_CONF_FILE --config-file /$Q_PLUGIN_CONF_FILE --config-file /$OVS_PLUGIN_CONF_FILE"
    screen_it q-agt-lb "sudo python $LB_AGENT_BINARY --config-file $Q_CONF_FILE --config-file /$Q_PLUGIN_CONF_FILE --config-file /$LB_PLUGIN_CONF_FILE"
else
    # Start the Quantum services
