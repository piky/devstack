...
ENABLED_SERVICES=${ENABLED_SERVICES:-g-api,g-reg,key,n-api,n-cpu,n-net,n-sch,n-nvnc,n-xvnc,n-cauth,horizon,mysql,rabbit,openstackx}
...
if [[ "$ENABLED_SERVICES" =~ "n-nvnc" ]]; then
...
if [[ "$ENABLED_SERVICES" =~ "n-nvnc" ]]; then
    screen_it n-nvnc "cd $NOVNC_DIR && ./utils/nova-novncproxy --flagfile $NOVA_DIR/bin/nova.conf --web ."
fi
if [[ "$ENABLED_SERVICES" =~ "n-xvnc" ]]; then
