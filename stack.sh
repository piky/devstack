...
if is_service_enabled quantum; then
    ...
    if [[ -e $QUANTUM_DIR/$Q_PLUGIN_CONF_FILE ]]; then
            sudo cp $QUANTUM_DIR/$Q_PLUGIN_CONF_FILE /$Q_PLUGIN_CONF_FILE
    fi
...
if is_service_enabled q-svc; then
    ...
    screen_it q-svc "cd $QUANTUM_DIR && python $QUANTUM_DIR/bin/quantum-server --config-file $Q_CONF_FILE --core_plugin=/$Q_PLUGIN_CONF_FILE"
fi
...
if is_service_enabled mysql && is_service_enabled nova; then
    if [[ "$NOVA_USE_QUANTUM_API" = "v1" ]]; then
        ...
        # Create a small network
        NET_ID=$(quantumv2 net-create --os_token $Q_ADMIN_USERNAME --os_url http://$Q_HOST:$Q_PORT --tenant_id $TENANT_ID net1 | grep ' id ' | get_field 2)

        # Create a subnet
        quantumv2 subnet-create --os_token $Q_ADMIN_USERNAME --os_url http://$Q_HOST:$Q_PORT --tenant_id $TENANT_ID --ip_version 4 --gateway  $NETWORK_GATEWAY $NET_ID $FIXED_RANGE
    fi
fi
