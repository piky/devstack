...
if is_service_enabled key; then
    ...
    if is_service_enabled swift && is_service_enabled_nova; then
        CREDS=$(keystone --os_username=nova --os_password=$SERVICE_PASSWORD --os_tenant_name=$SERVICE_TENANT ec2-credentials-create)
        ACCESS_KEY=$(echo "$CREDS" | awk '/ access / { print $4 }')
        SECRET_KEY=$(echo "$CREDS" | awk '/ secret / { print $4 }')
        add_nova_opt "s3_access_key=$ACCESS"
        add_nova_opt "s3_secret_key=$SECRET"
