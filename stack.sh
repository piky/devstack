...
if is_service_enabled horizon; then
    ...
    if [[ "$os_PACKAGE" = "deb" ]]; then
        ...
        if [[ "$(sestatus | awk '/status:/ { print $3 }')" != "disabled" ]]; then
            sudo semanage fcontext -a -e /var/log/httpd $DEST/logs 2>/dev/null ||: sudo restorecon -R -v $DEST/logs
        fi
    fi
...
if is_service_enabled swift; then
   ...
   # FIXME(dtroyer): verify if this is required on ubuntu
...
if is_service_enabled horizon; then
    screen_it horizon "cd $HORIZON_DIR && sudo tail -f $DEST/logs/apache_error.log"
fi
