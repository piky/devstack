    ...
    for lfile in $LRC; do
        if [[ "$lfile" == "localrc" ]]; then
            if [[ -r localrc ]]; then
                ...
                echo "# Generated file, do not exit" >.localrc.auto
                get_meta_section local.conf local $lfile >>.localrc.auto
            fi
        fi
    done
fi
...
# Phase: post-config
merge_config_group local.conf post-config
...
# Apply configuration from local.conf if it exists for layer 2 services
# Phase: extra
merge_config_group local.conf extra
