...
if is_suse; then
    ...
    if [[ $os_VENDOR = "openSUSE" && $os_RELEASE = "13.2" ]]; then
        sudo zypper --gpg-auto-import-keys -n ar -f http://download.opensuse.org/repositories/Cloud:/OpenStack:/Master/openSUSE_${os_RELEASE} "Cloud:OpenStack:Master.repo"
    elif [[ $os_VENDOR = "openSUSE" && $os_RELEASE != "13.2" ]]; then
        sudo zypper --gpg-auto-import-keys -n ar -f http://download.opensuse.org/repositories/Cloud:/OpenStack:/Master/openSUSE_Tumbleweed "Cloud:OpenStack:Master.repo"
    elif [[ $os_VENDOR = "SUSE LINUX" && $os_RELEASE = "12" ]]; then
        sudo zypper --gpg-auto-import-keys -n ar -f http://download.opensuse.org/repositories/Cloud:/OpenStack:/Master/SLE_${os_RELEASE} "Cloud:OpenStack:Master.repo"
    fi
fi
