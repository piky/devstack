...
# Launch nova-conductor
if is_service_enabled n-cond; then
    echo_summary "Starting nova-conductor"
    start_nova_process n-cond nova-conductor
fi

# Launch nova-cells
if is_service_enabled n-cell; then
    echo_summary "Starting nova-cells"
    start_nova_process n-cell-region nova-cells
    start_nova_process n-cell-child nova-cells
fi
...
if is_service_enabled neutron-api; then
    ...
    echo_summary "Starting nova-network"
    start_nova_network

    echo "Waiting for nova-network to be ready..."
    # NOTE(mriedem): We use the dns-domains command as that uses an RPC call
    # to the nova-network service.
    test_with_retry "nova dns-domains" "Failed to start nova-network" 30 1

    # Create a small network
    nova network-create --fixed-range-v4 $FIXED_RANGE "$PRIVATE_NETWORK_NAME"

    # Create some floating ips
    nova floating-ip-bulk-create --pool $PUBLIC_NETWORK_NAME $FLOATING_RANGE

    # Create a second pool
    nova floating-ip-bulk-create --pool $TEST_FLOATING_POOL $TEST_FLOATING_RANGE
fi
