...
if is_service_enabled keystone && is_service_enabled swift3 && is_service_enabled nova; then
    eval $(openstack ec2 credentials create --user nova --project $SERVICE_TENANT_NAME --user-domain $SERVICE_DOMAIN_NAME --project-domain $SERVICE_DOMAIN_NAME -f shell -c access -c secret)
