...
# on the command line
#
# Stop a specific process by specifying ``-s $SERVICE_SHUT_DOWN`` or
# ``--service $SERVICE_SHUT_DOWN`` on the command line.
# For example, unstack.sh -s g-reg.
...
# If -s SERVICE_SHUT_DOWN is specified, just shut down the specific service.
# If -s SERVICE_SHUT_DOWN is not specified, shut down all the services.
