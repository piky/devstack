...
DevStack is written in UNIX shell script, specifically ``bash``, although we
generally follow POSIX usage.  This choice was made because it best illustrates
the configuration steps that this implementation takes on setting up and
...
The original `DevStack repo`__ on GitHub contains the gh-pages branch that
supports the http://devstack.org site.  This is the primary DevStack
