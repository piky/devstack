...
# -b branch     The DevStack branch to check out (default is master; ignored if repo is not specified)
# -p            Push the resulting docs tree to the source repo; fatal error if repo is not specified
...
# .             Use the current repo and branch
...
# devstack.org is a GitHub gh-pages site in the cloudbuilders account
GH_PAGES_REPO=git@github.com:cloudbuilders/devstack.git

# Must use this shocco branch: https://github.com/dtroyer/shocco/tree/rst_support
...
if [[ "$1" == "." ]]; then
    REPO=""
    if [[ -n $PUSH_REPO ]]; then
        echo "We're not ready to allow this yet..."
        exit
    fi
else
    if [[ -z "$1" ]]; then
...
## Assumption is we are now in the DevStack repo workspace to process

# Pull the latest docs branch from devstack.org
rm -rf docs || true
git clone -b gh-pages $GH_PAGES_REPO docs

# Build list of scripts to process
FILES=""
for f in `find . -type f -name \*.sh -not -path \*shocco/\*`; do
    echo $f
    FILES+="$f "
...
if ! git diff-index HEAD --quiet && [[ -n $PUSH ]]; then
