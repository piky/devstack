...
if ! options=$(getopt -o hPAp:u:r:C: -l os-username:,os-password:,os-tenant-name:,os-tenant-id:,os-auth-url:,target-dir:,skip-tenant:,help -- "$@")
then
    #parse error
    exit 1
fi
...
do
    ...
    (*)  echo "$0: error - unexpected argument $1" >&2; exit 1 ;;
    esac
    shift
done
...
function add_entry(){
    ...
    nova --os-password "$user_passwd" --os-username "$user_name" --os-tenant-name "$tenant_name" x509-create-cert "$ec2_private_key" "$ec2_cert"
    cat >"$rcfile" <<EOF
...
function create_or_get_role(){
    local role_name=$1
    role_id=`keystone role-list| awk '/\|[[:space:]]*'"$role_name"'[[:space:]]*\|/ {print $2}'`
    if [ -n "$role_id" ]; then
        echo $role_id
    else
...
# Provides empty string when the uses does not exists
