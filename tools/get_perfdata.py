...
def get_latest_job(change, patchset=None, event_id=None):
    ...
    latest_job = None
    for job in data:
        if event_id and job['event_id'] != event_id:
            continue
        if not job['end_time']:
            continue
...
def get_job_urls(change, patchset=None, event_id=None):
...
if __name__ == '__main__':
    ...
    p.add_argument('--patchset', type=int, default=None,
                   help=('Patchset for which jobs will be taken '
                         '(default=latest)'))
    p.add_argument('--event_id', default=None,
                   help=('Event for which jobs will be taken '
                         '(useful for rehecked patches)'))
    args = p.parse_args()

    # If we're pulling logs from a rechecked patch, event_id will be
    # more specific. Otherwise, use the provided patchset, or the
    # latest, if unspecified.
    if args.event_id:
        job = get_latest_job(args.change, event_id=args.event_id)
        urls = get_job_urls(args.change, event_id=args.event_id)
    else:
        job = get_latest_job(args.change, patchset=args.patchset)
        urls = get_job_urls(args.change, patchset=job['patchset'])
