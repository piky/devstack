...
function verify_devstack_ipv6_setting {
    ...
    if ! python3 ${TOP_DIR}/tools/verify-ipv6-only-deployments.py "$_service_host"; then
        echo $SERVICE_HOST "SERVICE_HOST is not IPv6 which means devstack cannot deploy services on IPv6 addresses."
        exit 1
    fi
    if ! python3 ${TOP_DIR}/tools/verify-ipv6-only-deployments.py "$_host_ipv6"; then
        echo $HOST_IPV6 "HOST_IPV6 is not IPv6 which means devstack cannot deploy services on IPv6 addresses."
        exit 1
    fi
    if ! python3 ${TOP_DIR}/tools/verify-ipv6-only-deployments.py "$_service_listen_address"; then
    ...
    if ! python3 ${TOP_DIR}/tools/verify-ipv6-only-deployments.py "$_service_local_host"; then
        echo $SERVICE_LOCAL_HOST "SERVICE_LOCAL_HOST is not IPv6 which means devstack cannot deploy services on IPv6 addresses."
        exit 1
    fi
    if ! python3 ${TOP_DIR}/tools/verify-ipv6-only-deployments.py "$_tunnel_endpoint_ip"; then
...
function verify_service_listen_address_is_ipv6 {
    ...
    for endpoint in ${endpoints}; do
        ...
        if ! python3 ${TOP_DIR}/tools/verify-ipv6-only-deployments.py "$endpoint_address"; then
            all_ipv6=False
