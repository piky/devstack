...
EC2_URL=$(openstack endpoint show -f shell -c publicurl ec2| sed -r 's/publicurl=\"(.+)\"/\1/g')
if [[ -z $EC2_URL ]]; then
    EC2_URL=http://localhost:8773/services/Cloud
fi

S3_URL=$(openstack endpoint show -f shell -c publicurl s3| sed -r 's/publicurl=\"(.+)\"/\1/g')
if [[ -z $S3_URL ]]; then
