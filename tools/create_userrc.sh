...
# looks like I can't ask for all tenant related to a specified user
    openstack project list --long -f csv | grep ',True' | grep -v ",${SKIP_TENANT}," | while IFS=, read tenant_id tenant_name desc enabled; do
        if echo $SKIP_TENANT| grep -q ",$tenant_name,"; then
            continue;
        fi

        openstack user list --project $tenant_id --long -f csv | grep ',True' | grep -v ",${SKIP_TENANT}," | while IFS=, read user_id user_name project email enabled; do
