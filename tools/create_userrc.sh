...
-C <tenent_name> create user and tenant, the specifid tenant will be the user's tenant
...
# Supporting different tenent for services is out of scope.
...
publicurl=''
eval $(openstack endpoint show --all ec2 -f shell -c publicurl)
if [[ -z $publicurl ]]; then
    publicurl=http://localhost:8773/service/Cloud
fi
EC2_URL=$publicurl

publicurl=''
eval $(openstack endpoint show --all s3 -f shell -c publicurl)
if [[ -z $publicurl ]]; then
...
function get_user_id {
    openstack user list | grep " $1 " | get_field 1
}

if [ $MODE != "create" ]; then
# looks like I can't ask for all tenant related to a specified user
    for tenant_id_at_name in `openstack project list --long | awk 'BEGIN {IGNORECASE = 1} /true[[:space:]]*\|$/ {print  $2 "@" $4}'`; do
