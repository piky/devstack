...
if [ -z "$templateuuid" ]; then
	...
	cp -f $THIS_DIR/devstackubuntu_latecommand.sh $HTTP_SERVER_LOCATION/latecommand.sh
	    ...
	    -e "s,\(d-i preseed/late_command string\).*,\1 in-target mkdir -p /tmp; in-target wget --no-proxy ${HOST_IP}/latecommand.sh -O /tmp/latecommand.sh; in-target bash /tmp/latecommand.sh,g" \
