...
# It does only a net install only
#
# Based on a script by: David Markey <david.markey@citrix.com>
#
# Not these versions of ubuntu are not officially supported on
# XenServer 6.0.2 and below.
...
previous_template=$(xe template-list name-label="$UBUNTU_INST_TEMPLATE_NAME" params=uuid --minimal)
...
new_uuid=$(xe vm-clone uuid=$builtin_uuid new-name-label="$UBUNTU_INST_TEMPLATE_NAME")
...
pvargs="-- quiet console=hvc0 partman/default_filesystem=ext3 console-setup/ask_detect=false"
pvargs="${pvargs} locale=${UBUNTU_INST_LOCALE}"
pvargs="${pvargs} keyboard-configuration/layoutcode=${UBUNTU_INST_KEYBOARD}"
pvargs="${pvargs} netcfg/choose_interface=${HOST_IP_IFACE}"
pvargs="${pvargs} netcfg/get_hostname=os netcfg/get_domain=os auto"
pvargs="${pvargs} url=${preseed_url}"

if [ "$NETINSTALLIP" != "dhcp" ]; then
    netcfgargs="netcfg/disable_autoconfig=true"
    netcfgargs="${netcfgargs} netcfg/get_nameservers=${UBUNTU_INST_NAMESERVERS}"
    netcfgargs="${netcfgargs} netcfg/get_ipaddress=${UBUNTU_INST_IP}"
    netcfgargs="${netcfgargs} netcfg/get_netmask=${UBUNTU_INST_NETMASK}"
    netcfgargs="${netcfgargs} netcfg/get_gateway=${UBUNTU_INST_GATEWAY}"
    netcfgargs="${netcfgargs} netcfg/confirm_static=true"
