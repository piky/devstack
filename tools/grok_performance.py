...
def sanity_check(data):
    """Sanity check a dataset.

    This should be things we can look at and know something was wrong,
    like overflowing a counter, missing data, etc.
    """
    count = sum(db['queries'] for db in data['db'])
    # FIXME: This should really be pulled from somewhere common
    LOG.debug('Total DB queries: %i' % count)
    assert count != 100000, 'Database query overflow!'
