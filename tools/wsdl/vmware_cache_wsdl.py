...
"""
A tool to help you cache vSphere WSDL locally.

Use with vSphere 5.0 and OpenStack nova compute driver Grizzly or later.

If you are using OpenStack Nova compute driver with grizzly or havana along
side vSphere API version 5.0 or earlier you may have a WSDL parse exception
on driver initialization. This is due to the sensitivity of the Python XML
parser. It will not tolerate missing XML or slightly malformed XML. Version
5.1 and later of vCenter have addressed this issue and made their WSDL files
Python friendly.

Targets a supplied vSphere server (vCenter or naked ESXi) and downloads the
WSDL off of it. Substitutes missing XML content with parser friendly stubs.
"""
...
for wsdl_file in WSDL_FILES:
    ...
    if code == 200:
        ...
        print "404: fixing missing %s" % wsdl_file
