...
# Check if the QEMU binary has already been compiled
# The variable QEMU_DIR is specific for a given QEMU_VERSION
if [[ ! -f "$QEMU_DIR/`uname -m`-softmmu/qemu-system-`uname -m`" ]]; then
    ...
    echo "QEMU $1 binary found in $QEMU_DIR, skipping compilation."
fi
