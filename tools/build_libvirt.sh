...
# Check if the libvirtd binary has already been compiled
# The variable LIBVIRT_DIR is specific for a given LIBVIRT_VERSION
if [[ ! -f  "$LIBVIRT_DIR"/daemon/libvirtd ]]; then
    ...
    echo "libvirtd $1 binary found in $LIBVIRT_DIR, skipping compilation."
fi
