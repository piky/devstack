...
if [[ ! -d "$LIBVIRT_DIR" || "$RECLONE" = "yes" ]]; then
    ...
    cd "$LIBVIRT_DIR"
    ./configure --prefix="$LIBVIRT_QEMU_DEV_INSTALL" --localstatedir=/var \
        --libdir="$LIBVIRT_QEMU_DEV_INSTALL"/lib --sysconfdir=/etc \
...
if is_ubuntu; then
    if [[ ! -f /etc/init/libvirtd.conf ]]; then
        ...
        sudo sed -i "s#exec \/usr\/sbin#"$(echo $LIBVIRT_QEMU_DEV_INSTALL | sed 's#\/#\\\/#g')"\/sbin#g" /etc/init/libvirtd.conf
        sudo sed -i "41i\
                export PATH="$LIBVIRT_QEMU_DEV_INSTALL"/sbin:"$LIBVIRT_QEMU_DEV_INSTALL"/bin:"$PATH"\nexport LD_LIBRARY_PATH="$LIBVIRT_QEMU_DEV_INSTALL"lib:"$LD_LIBRARY_PATH"" /etc/init/libvirtd.conf
    ...
    if [[ -f /usr/lib/systemd/system/libvirtd.service ]]; then
        sudo sed -i "s#ExecStart=\/usr\/#ExecStart="$(echo $LIBVIRT_QEMU_DEV_INSTALL | sed 's#\/#\\\/#g')"\/#g" /usr/lib/systemd/system/libvirtd.service
        FEDORA_ENV_FILE=`sudo cat /usr/lib/systemd/system/libvirtd.service | grep EnvironmentFile | awk 'BEGIN {FS = "-"}  {print $2}'`
        echo PATH="$LIBVIRT_QEMU_DEV_INSTALL"/sbin:"$LIBVIRT_QEMU_DEV_INSTALL"/bin:"$PATH" | sudo tee -a $FEDORA_ENV_FILE
        echo LD_LIBRARY_PATH="$LIBVIRT_QEMU_DEV_INSTALL"/lib:"$LD_LIBRARY_PATH" | sudo tee -a $FEDORA_ENV_FILE
