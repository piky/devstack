...
if [[ ! -d "$LIBVIRT_DIR" || "$RECLONE" = "yes" ]]; then
    ...
    cd "$LIBVIRT_DIR"
    ./configure --prefix="$HYP_INSTALL_DIR" --localstatedir=/var \
        --libdir="$HYP_INSTALL_DIR"/lib --sysconfdir=/etc \
...
if is_ubuntu; then
    if [[ ! -f /etc/init/libvirtd.conf ]]; then
        ...
        sudo sed -i "s#exec \/usr\/sbin#"$(echo $HYP_INSTALL_DIR | sed 's#\/#\\\/#g')"\/sbin#g" /etc/init/libvirtd.conf
        sudo sed -i "41i\
                export PATH="$HYP_INSTALL_DIR"/sbin:"$HYP_INSTALL_DIR"/bin:"$PATH"\nexport LD_LIBRARY_PATH="$HYP_INSTALL_DIR"lib:"$LD_LIBRARY_PATH"" /etc/init/libvirtd.conf
    ...
    if [[ -f /usr/lib/systemd/system/libvirtd.service ]]; then
        ...
        echo PATH="$HYP_INSTALL_DIR"/sbin:"$HYP_INSTALL_DIR"/bin:"$PATH" | sudo tee -a $FEDORA_ENV_FILE
        echo LD_LIBRARY_PATH="$HYP_INSTALL_DIR"/lib:"$LD_LIBRARY_PATH" | sudo tee -a $FEDORA_ENV_FILE
