...
# testing. Here we simply look for AMI images stored in Glance
# and set the appropriate variables for use in the tempest config
# TODO(jaypipes) Support more than AMI images when Tempest does...
# Get the image UUID
IMAGE_UUID=$(glance index disk_format=ami | grep ami | cut -d' ' -f1)
