...
# Clean up DevStack files/directories (inside DevStack's top directory only)
...
# This prefixes all filenames with the DevStack path ($TOP_DIR)
DEVSTACK_TO_CLEAN=$(echo $DEVSTACK_TO_CLEAN | sed "s/^\| / \$TOP_DIR\//g")
